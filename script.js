...
function mostrarReservas(lista) {
  ...
  lista.forEach((r) => {
    ...
    fila.innerHTML = `
      <td><strong>${r.cliente}</strong><br><small>${r.telefono}</small></td>
      <td>${formatearFecha(r.fecha)}</td>
      <td>${r.hora_inicio} - ${r.hora_fin}</td>
      <td>${r.personas}</td>
      <td>${r.mesa}</td>
      <td><span class="estado">${r.estado}</span></td>
      <td><button class="eliminar" onclick="eliminarReserva(${r.id})">Eliminar</button></td>
    `;
...
function crearCalendario() {
  ...
  for (let i = 1; i <= 30; i++) {
    const fecha = `2026-06-${String(i).padStart(2, "0")}`;
    const tiene = reservas.some(r => String(r.fecha).slice(0, 10) === fecha);

    const dia = document.createElement("div");
    dia.textContent = i;
    dia.className = tiene ? "dia con-reserva" : "dia";
    dia.onclick = () => mostrarReservasDia(fecha, dia);
...
function mostrarReservasDia(fecha, elemento) {
  ...
  document.getElementById("fechaSeleccionada").textContent = `Reservas para ${fecha}`;

  const contenedor = document.getElementById("reservasDelDia");
  const delDia = reservas.filter(r => String(r.fecha).slice(0, 10) === fecha);
  ...
  contenedor.innerHTML = delDia.map(r => `
    <div class="item-dia">
      <strong>${r.cliente}</strong><br>
      ${r.hora_inicio} - ${r.hora_fin} | ${r.mesa} | ${r.personas}
    </div>
  `).join("");
}
